...
RegisterServerEvent('esx_fbijob:haILovePizzandcuff')
AddEventHandler('esx_fbijob:haILovePizzandcuff', function(target)
	local xPlayer = ESX.GetPlayerFromId(source)

	if xPlayer.job.name == 'fbi' then
		TriggerClientEvent('esx_fbijob:haILovePizzandcuff', target)
	else
...
ESX.RegisterServerCallback('esx_fbijob:buyJobVehicle', function(source, cb, vehicleProps, type)
	...
	if price == 0 then
		...
		if xPlayer.getMoney() >= price then
			...
			MySQL.Async.execute('INSERT INTO owned_vehicles (owner, vehicle, plate, type, job, `stored`, date, paidprice, tunerdata) VALUES (@owner, @vehicle, @plate, @type, @job, @stored, @date, @paidprice, @tunerdata)', {
